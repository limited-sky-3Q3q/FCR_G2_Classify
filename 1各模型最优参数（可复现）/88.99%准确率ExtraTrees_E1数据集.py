...
print("="*80)
print("复现88.99%准确率ExtraTrees模型 - E1数据集")
print("="*80)
...
# 5. 模型训练
print("\n【5. 模型训练】")
et = ExtraTreesClassifier(**model_params)
et.fit(X_selected, y_binary)
print("模型训练完成!")
...
scores = cross_val_score(et, X_selected, y_binary, cv=cv, scoring='accuracy', n_jobs=-1)
...
model_path = os.path.join(save_dir, '88.99%准确率ExtraTrees_E1数据集.joblib')
joblib.dump(model_info, model_path)
print(f"模型已保存到: {model_path}")

info_path = os.path.join(save_dir, '88.99%准确率ExtraTrees_E1数据集_信息.txt')
with open(info_path, 'w', encoding='utf-8') as f:
    f.write(f"ExtraTrees模型信息 - E1数据集\n")
    f.write("="*70 + "\n\n")
    f.write(f"准确率: 88.99%\n")
    f.write(f"标准差: ±9.59%\n\n")
    f.write("模型参数:\n")
