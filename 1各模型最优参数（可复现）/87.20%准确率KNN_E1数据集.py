...
knn.fit(X_selected, y_binary)
print("模型训练完成!")

# 6. 特征重要性
print("\n【6. 特征重要性】")
print("KNN模型没有直接的特征重要性指标")

# 7. 交叉验证
print("\n【7. 交叉验证】")
cv = StratifiedKFold(n_splits=8, shuffle=True, random_state=42)
scores = cross_val_score(knn, X_selected, y_binary, cv=cv, scoring='accuracy', n_jobs=-1)
...
model_path = os.path.join(save_dir, '87.20%准确率KNN_E1数据集.joblib')
joblib.dump(model_info, model_path)
print(f"模型已保存到: {model_path}")

info_path = os.path.join(save_dir, '87.20%准确率KNN_E1数据集_信息.txt')
with open(info_path, 'w', encoding='utf-8') as f:
    f.write(f"KNN模型信息 - E1数据集\n")
    f.write("="*70 + "\n\n")
    f.write(f"准确率: 87.20%\n")
    f.write(f"标准差: ±11.23%\n\n")
    f.write("模型参数:\n")
