...
print("="*80)
print("复现85.12%准确率RandomForest模型 - E1数据集")
print("="*80)
...
scores = cross_val_score(rf, X_selected, y_binary, cv=cv, scoring='accuracy', n_jobs=-1)
...
model_path = os.path.join(save_dir, '85.12%准确率RandomForest_E1数据集.joblib')
joblib.dump(model_info, model_path)
print(f"模型已保存到: {model_path}")

info_path = os.path.join(save_dir, '85.12%准确率RandomForest_E1数据集_信息.txt')
with open(info_path, 'w', encoding='utf-8') as f:
    f.write(f"RandomForest模型信息 - E1数据集\n")
    f.write("="*70 + "\n\n")
    f.write(f"准确率: 85.12%\n")
    f.write(f"标准差: ±10.15%\n\n")
    f.write("模型参数:\n")
