...
# 5. 模型训练
print("\n【5. 模型训练】")
svc = SVC(**model_params, probability=True)
svc.fit(X_selected, y_binary)
print("模型训练完成!")

# 6. 特征重要性
print("\n【6. 特征重要性】")
# 线性SVM可以获取coef_作为特征重要性
if svc.kernel == 'linear':
    ...
    print("非线性核函数无法直接获取特征重要性")

# 7. 交叉验证
print("\n【7. 交叉验证】")
cv = StratifiedKFold(n_splits=8, shuffle=True, random_state=42)
scores = cross_val_score(svc, X_selected, y_binary, cv=cv, scoring='accuracy', n_jobs=-1)
...
model_info = {
    ...
    'feature_importance': feature_importance if svc.kernel == 'linear' else None,
    'cv_scores': scores
}

model_path = os.path.join(save_dir, '88.99%准确率SVM_E1数据集.joblib')
joblib.dump(model_info, model_path)
print(f"模型已保存到: {model_path}")

info_path = os.path.join(save_dir, '88.99%准确率SVM_E1数据集_信息.txt')
with open(info_path, 'w', encoding='utf-8') as f:
    f.write(f"SVM模型信息 - E1数据集\n")
    f.write("="*70 + "\n\n")
    f.write(f"准确率: 88.99%\n")
    f.write(f"标准差: ±9.59%\n\n")
    f.write("模型参数:\n")
